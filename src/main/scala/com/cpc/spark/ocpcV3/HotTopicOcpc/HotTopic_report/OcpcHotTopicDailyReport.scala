...
object OcpcHotTopicDailyReport {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName(s"ocpc hottopic hourly summary: $date, $hour").enableHiveSupport().getOrCreate()

    val summaryReport = getHourlyReport(date, hour, spark)
    val tableName = "dl_cpc.ocpcv3_hottopic_report_summary_hourly"
  ...
  def getHourlyReport(date: String, hour: String, spark: SparkSession) = {
    ...
    val sqlRequest2 =
      s"""
         ...
         |    SUM(case when is_cpa_ok=0 and is_step2=1 then 1 else 0 end) as high_cpa_adnum,
         |    SUM(case when is_step2=1 then cost else 0 end) as step2_cost,
         |    SUM(case when is_step2=1 and is_cpa_ok=0 then (cost - cpa_given * conversion * 1.2) else 0 end) as step2_cpa_high_cost,
         |    SUM(impression) as impression,
         |    SUM(click) as click,
         |    SUM(conversion) as conversion,
         |    SUM(cost) as cost
