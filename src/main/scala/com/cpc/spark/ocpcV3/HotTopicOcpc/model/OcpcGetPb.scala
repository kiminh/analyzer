...
object OcpcGetPb {
  def main(args: Array[String]): Unit = {
    ...
    val base      = getBaseData( mediaSelection, date, hour, spark )  // 返回ocpc_ctr_data_hourly中date日，hour时之前7天内出现的identifier
    val cvrData   = getOcpcCVR(  mediaSelection, conversionGoal, date, hour, spark ) // 返回过去7天每个identifier对应的转化数
    ...
    resultDF.write.mode("overwrite").insertInto( "test.ocpc_prev_pb_hourly"  ) //只有版本分区
  ...
  def getBaseData(mediaSelection: String, date: String, hour: String, spark: SparkSession) = {
    // 取历史数据
    /**
      * 返回ocpc_ctr_data_hourly中date日，hour时之前7天内出现的identifier
      * */
  ...
  def getInitK(mediaSelection: String, conversionGoal: Int, date: String, hour: String, spark: SparkSession) = {
    ...
    val sqlRequest1 =
      s"""
       ...
       """.stripMargin
    println(sqlRequest1)
    val ctrData = spark //过去7天identifier对应的total_cost, ctrcnt, total_pcvr
      .sql(sqlRequest1)
  ...
  def getOcpcCVR(mediaSelection: String, conversionGoal: Int, date: String, hour: String, spark: SparkSession) = {
    ...
    println(sqlRequest)
    val rawCvr = spark.sql(sqlRequest)  //searchid, label

    // 数据汇总
    val resultDF = ocpcUnionlog  //search_id, identifier
