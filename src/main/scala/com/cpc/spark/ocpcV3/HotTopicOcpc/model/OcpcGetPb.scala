...
object OcpcGetPb {
  def main(args: Array[String]): Unit = {
    ...
    // 读取数据
    val base = getBaseData(date, hour, spark)          // 返回ocpc_ctr_data_hourly   中date日，hour时之前7天的identifier, adclass
    val cvrData = getCVR(  date, hour, spark)          // 返回ocpc_union_log_hourly  中date日，hour时过去7天内isclick = 1的searchid， identifier
    val cpaHistory = getCPAgiven( date, hour, spark )  // 返回ocpc_cpa_history_hourly中date日，hour时，版本为v1的数据：identifier, new_adclass, cpa_src, cpa_history, conversion_goal等
    val kvalue = getK(cpaHistory, date, hour, spark )  // 返回pidk和regressionk比较之后最终确定的identifier, conversion_goal, kvalue
    val adclassCPA = spark //ocpc_cpa_history_adclass_hourly中date日、hour时、v1版本的new_adclass, cpa_adclass
  ...
  def getBaseData(date: String, hour: String, spark: SparkSession) = {
    // 取历史数据
    /**
      * 返回ocpc_ctr_data_hourly中date日，hour时之前7天的identifier, adclass
      * */
    ...
    resultDF // identifier, new_adclass
  }
  ...
  def getCVR(date: String, hour: String, spark: SparkSession) = {
    ...
    // 取历史数据
    /**
      * 返回ocpc_union_log_hourly中date日，hour时过去7天内isclick = 1的searchid， identifier
      */
    val dateConverter = new SimpleDateFormat("yyyy-MM-dd")
    val today = dateConverter.parse(date)
    val calendar = Calendar.getInstance
  ...
  def getCPAgiven(date: String, hour: String, spark: SparkSession) = {
    /**
      * 返回ocpc_cpa_history_hourly中date日，hour时，版本为v1的数据：identifier, new_adclass, cpa_src, cpa_history, conversion_goal等
  ...
  def getK(cpaGiven: DataFrame, date: String, hour: String, spark: SparkSession) = {
    /**
      * 返回pidk和regressionk比较之后最终确定的identifier, conversion_goal, kvalue
    ...
    resultDF // identifier, conversion_goal, kvalue
