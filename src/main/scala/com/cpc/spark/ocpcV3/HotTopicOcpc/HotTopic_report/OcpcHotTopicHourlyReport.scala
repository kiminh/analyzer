...
object OcpcHotTopicHourlyReport {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result1 = result
      ...
      .withColumn("auc", lit(0)).select( "unitid",
      "userid","conversion_goal","step2_click_percent","is_step2", "cpa_given", "cpa_real", "cpa_ratio", "is_cpa_ok", "impression","click",
  ...
  def calculateData(data: DataFrame, date: String, hour: String, spark: SparkSession) = {
    val result = data
      ...
      .withColumn("is_cpa_ok", when(col("cpa_ratio")>=0.95, 1).otherwise(0))
