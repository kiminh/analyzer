...
//    cvrCali.write.mode("overwrite").saveAsTable("test.check_ocpc_new_calidata20190411")
    ...
    val result = getRecommendLabelNew(updateData, date, hour, spark)
...
//      .repartition(10).write.mode("overwrite").saveAsTable("test.ocpc_suggest_cpa_recommend_hourly20190411")
  ...
  def getRecommendLabelNew(data: DataFrame, date: String, hour: String, spark: SparkSession) = {
    val resultDF = data
      ...
      .withColumn("is_recommend", when(col("auc") <= 0.58, 0).otherwise(col("is_recommend")))
