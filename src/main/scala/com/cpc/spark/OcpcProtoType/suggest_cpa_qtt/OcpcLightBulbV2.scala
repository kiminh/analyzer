...
//      .repartition(5).write.mode("overwrite").saveAsTable("test.ocpc_qtt_light_control_version20190415")
  ...
  def getCPAgivenV2(date: String, hour: String, spark: SparkSession) = {
    ...
    val table = "(select id, user_id, ideas, bid, ocpc_bid, ocpc_bid_update_time, cast(conversion_goal as char) as conversion_goal, status from adv.unit where ideas is not null and target_medias = '80000001,80000002') as tmp"
