...
object OcpcPIDerror {
  def main(args: Array[String]): Unit = {
    ...
    val cvrData = calculateCvr(baseData, date, hour, spark)
    val errorData = calculateError(cvrData, minCV, date, hour, spark)
  ...
  def calculateCvr(baseData: DataFrame, date: String, hour: String, spark: SparkSession) = {
    val data = baseData
      ...
      .agg(
        ...
        avg(col("iscvr")).alias("post_cvr")
      )
      .select("unitid", "click", "pre_cvr", "post_cvr", "cv")
      .cache()
  ...
  def calculateError(baseData: DataFrame, minCV: Int, date: String, hour: String, spark: SparkSession) = {
    ...
    val sqlRequest1 =
      s"""
         ...
         |  cv > $minCV
       """.stripMargin
    val currentError = spark
      .sql(sqlRequest1)
      .withColumn("current_error", udfCalculateError()(col("post_cvr"), col("pre_cvr")))
      .cache()

    currentError.show(10)
    currentError
  }

  def udfCalculateError() = udf((post_cvr: Double, pre_cvr: Double) => {
    val result = 1.0 - pre_cvr / post_cvr
    result
  })
