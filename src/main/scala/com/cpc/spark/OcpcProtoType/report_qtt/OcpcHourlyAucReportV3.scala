...
object OcpcHourlyAucReportV3 {
  def main(args: Array[String]): Unit = {
    ...
    val rawData = getOcpcLogV2(media, date, hour, spark).filter(s"is_hidden = $isHidden")
...
//    conversionData.write.mode("overwrite").saveAsTable("test.ocpc_summary_report_hourly20190226")
  ...
  def getOcpcLogV2(media: String, date: String, hour: String, spark: SparkSession) = {
    ...
    val resultDF = ctrData
      ...
      .select("searchid", "unitid", "userid", "isclick", "isshow", "exp_cvr", "cpagiven", "bid", "conversion_goal", "price", "iscvr1", "iscvr2", "iscvr3", "iscvr4", "is_hidden")
