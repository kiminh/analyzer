...
/*2019-05-14 基础表slim_union_log 换成dl_cpc.cpc_basedata_union_events*/
...
//  每日更新前一天24小时的段子数据
    ...
    val  threstab1 = spark.sql(
      s"""
           |insert   overwrite table dl_cpc.duanzi_ecpm_threshold_qbj partition (dt='${yesdate}',traffic=${traffic1})
       ...
       """.stripMargin).selectExpr("adslot_id","hour","adclass","threshold","dt").
      toDF("adslot_id","hour","adclass","threshold","dt")
    ...
    val  threstab2 = spark.sql(
      s"""
         |insert   overwrite table dl_cpc.duanzi_ecpm_threshold_qbj partition (dt='${yesdate}',traffic=${traffic2})
         |select   nd.adslot_id,nd.hour,nd.adclass, max(nd.ecpm)
         |from
         |(
         |select  '${yesdate}' as dt,adslot_id, hour, adclass,unitid,ecpm,
 ...
 |)  hd
    ...
    println(s"dl_cpc.duanzi_ecpm_threshold_qbj ${traffic2} insert success")

    //-----阈值试算 traffic3----------
    val  threstab3 = spark.sql(
      s"""
         |insert   overwrite table dl_cpc.duanzi_ecpm_threshold_qbj partition (dt='${yesdate}',traffic=${traffic3})
         |select   nd.adslot_id,nd.hour,nd.adclass, max(nd.ecpm)
         |from
         |(
         |select  '${yesdate}' as dt,adslot_id, hour, adclass,unitid,ecpm,
 ...
 |)  hd
    ...
    println(s"dl_cpc.duanzi_ecpm_threshold_qbj ${traffic3} insert success")
    ...
    val  valitab1 = spark.sql(
       s"""
          ...
          |    where  dt='${yesdate}'  and traffic =${traffic1}
          ...
          |group by nd.adslot_id,nd.hour,nd.adclass,
          |         case when expcpm<threshold    then '3ddrop>${traffic1}'
          |             when expcpm>=threshold   then '3dkeep<=${traffic1}' end
         """.stripMargin).selectExpr("adslot_id","hour","adclass", "cate","nd_query","nd_consume","nd_cpq","nd_pcoc",
                s"""'${yesdate}' as dt""",s"""${traffic1} as traffic""")
    ...
    val checktab=spark.sql(
      s"""
         |select  cate,sum(nd_query),sum(nd_consume)
         |from    dl_cpc.duanzi_ecpm_datadis_qbj
         |where   dt='${yesdate}'
         |and     cate like '%${traffic1}%'
         |group by cate;
      """.stripMargin).selectExpr("cate","query","consume",s"'${yesdate}' as dt",s"""'${traffic1}' as traffic""").
    ...
    val  valitab2 = spark.sql(
      s"""
         ...
         |    select  dt,adslot_id,hour,adclass,threshold
         |    from     dl_cpc.duanzi_ecpm_threshold_qbj
         |    where  dt='${yesdate}'  and traffic =${traffic2}
         |    group by dt,adslot_id,hour,adclass,threshold
         |) nd2
         |on  nd2.dt=nd.dt
         |and nd2.adslot_id=nd.adslot_id
         ...
         |group by nd.adslot_id,nd.hour,nd.adclass,
         |         case when expcpm<threshold    then '3ddrop>${traffic2}'
         |             when expcpm>=threshold   then '3dkeep<=${traffic2}' end
         """.stripMargin).selectExpr("adslot_id","hour","adclass","cate","nd_query","nd_consume","nd_cpq","nd_pcoc",
      s"""'${yesdate}' as dt""",s"""${traffic2} as traffic""")
...
//  -------测算请求&消耗占比-------
    val checktab2=spark.sql(
      s"""
         ...
         |from    dl_cpc.duanzi_ecpm_datadis_qbj
         |where   dt='${yesdate}'
         |and     cate like '%${traffic2}%'
         |group by cate;
      """.stripMargin).selectExpr("cate","query","consume",s"'${yesdate}' as dt",s"""'${traffic2}' as traffic""").
      toDF("cate","query","consume","dt","traffic")
    checktab.show(10,false)
    ...
    val  valitab3 = spark.sql(
      s"""
         ...
         |    select  dt,adslot_id,hour,adclass,threshold
         |    from     dl_cpc.duanzi_ecpm_threshold_qbj
         |    where  dt='${yesdate}'  and traffic =${traffic3}
         |    group by dt,adslot_id,hour,adclass,threshold
         |) nd2
         |on  nd2.dt=nd.dt
         |and nd2.adslot_id=nd.adslot_id
         ...
         |group by nd.adslot_id,nd.hour,nd.adclass,
         |         case when expcpm<threshold    then '3ddrop>${traffic3}'
         |             when expcpm>=threshold   then '3dkeep<=${traffic3}' end
         """.stripMargin).selectExpr("adslot_id","hour","adclass","cate","nd_query","nd_consume","nd_cpq","nd_pcoc",
      s"""'${yesdate}' as dt""",s"""${traffic3} as traffic""")
    ...
    val checktab3=spark.sql(
      s"""
         ...
         |from    dl_cpc.duanzi_ecpm_datadis_qbj
         |where   dt='${yesdate}'
         |and     cate like '%${traffic3}%'
         |group by cate;
      """.stripMargin).selectExpr("cate","query","consume",s"'${yesdate}' as dt",s"""'${traffic3}' as traffic""").
      toDF("cate","query","consume","dt","traffic")
    checktab.show(10,false)
