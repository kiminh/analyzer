...
object OcpcGetPb_realtimev2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 计算计费比系数模块
    val jfbData_baseRaw = OcpcJFBfactor_base(date, hour, expTag, dataRaw, hourInt1, hourInt2, hourInt3, spark)
    val jfbData_base = jfbData_baseRaw
      .withColumn("jfb_factor", lit(1.0) / col("jfb"))
      .selectExpr("cast(unitid as string) identifier", "conversion_goal", "media", "jfb_factor as jfb_factor1")
      .cache()
    jfbData_base.show(10)

    // 数据关联
    val jfbData_final = jfbData
      .join(jfbData_base, Seq("identifier", "conversion_goal", "media" ), "outer")
      .withColumn("jfb_factor", when(col("jfb_factor2").isNull, col("jfb_factor1")).otherwise(col("jfb_factor2")))
    ...
    val data = assemblyData(jfbData_final, pcocData_final, bidFactor, expTag, spark)
 ...
 */
  ...
  计费比系数模块
  */
  def OcpcJFBfactor_base(date: String, hour: String, expTag: String, dataRaw: DataFrame, hourInt1: Int, hourInt2: Int, hourInt3: Int, spark: SparkSession) = {
    // smooth实验配置文件
    ...
    val resultDF = calibration
      .select("unitid", "conversion_goal", "media", "jfb")
  ...
  def calculateCalibrationValueJFB(dataRaw1: DataFrame, dataRaw2: DataFrame, dataRaw3: DataFrame, spark: SparkSession) = {
    ...
    val data1 = dataRaw1
      .filter(s"cv >= min_cv")
      .select("unitid", "conversion_goal", "media", "jfb", "priority")
