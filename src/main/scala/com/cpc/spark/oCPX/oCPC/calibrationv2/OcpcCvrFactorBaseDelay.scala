package com.cpc.spark.oCPX.oCPC.calibrationv2
...
object OcpcCvrFactorBaseDelay {
  def main(args: Array[String]): Unit = {
    ...
    val result = OcpcCvrFactorBaseDelayMain(date, hour, version, expTag, hourInt, spark)
    result
      .repartition(10).write.mode("overwrite").saveAsTable("test.check_cvr_factor20190723a")
  }

  def OcpcCvrFactorBaseDelayMain(date: String, hour: String, version: String, expTag: String, hourInt: Int, spark: SparkSession) = {
    /*
    动态计算alpha平滑系数
    1. 基于原始pcoc，计算预测cvr的量纲系数
    2. 二分搜索查找到合适的平滑系数
