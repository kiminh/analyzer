...
object OcpcHourlyReport {
  ...
  def saveBaseDataToHDFS(data: DataFrame, date: String, hour: String, spark: SparkSession) = {
    val resultDF = data
      ...
      .select("ideaid", "unitid", "userid", "adclass", "conversion_goal", "industry", "media", "show", "click", "cv", "total_price", "total_bid", "total_precvr", "total_prectr", "total_cpagiven", "total_jfbfactor", "total_cvrfactor", "total_calipcvr", "total_calipostcvr", "total_cpasuggest", "total_smooth_factor", "is_hidden", "adslot_type", "total_exp_cpm", "total_rawcvr", "deep_conversion_goal", "check_cpa_priority", "date", "hour")
      .filter(s"date is not null and hour is not null")
      .na.fill(0, Seq("impression", "click", "cv", "total_price", "total_bid", "total_precvr", "total_prectr", "total_cpagiven", "total_jfbfactor", "total_cvrfactor", "total_calipcvr", "total_calipostcvr", "total_cpasuggest", "total_smooth_factor", "is_hidden", "adslot_type", "total_exp_cpm", "total_rawcvr", "deep_conversion_goal", "check_cpa_priority"))
  ...
  def calculateBaseData(rawData: DataFrame, spark: SparkSession) = {
    ...
    val sqlRequest =
      s"""
         ...
         |FROM
         |  raw_data
         |GROUP BY ideaid, unitid, userid, adclass, adslot_type, conversion_goal, deep_conversion_goal, check_cpa_priority, industry, media, hr, cast(ocpc_log_dict['IsHiddenOcpc'] as int)
       """.stripMargin
    println(sqlRequest)
