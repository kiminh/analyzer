...
object LRTransform {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder()
      ...
      .appName("lr -> xgboost %s %s %s".format(action, parser, dataType))
      .enableHiveSupport()
      .getOrCreate()

    if (action == "eval") {
      evaluate(spark, dataType, mlmfile)
      System.exit(1)
    } else if (action == "stub") {
      testServer(spark, dataType, mlmfile)
      System.exit(1)
    }

    //按分区取数据
    val ctrPathSep = getPathSeq(days.toInt)
    initFeatureDict(spark, ctrPathSep)
    val userAppIdx = getUidApp(spark, ctrPathSep)
    var qtt = getData(spark,dataSrc,ctrPathSep).coalesce(600)
    ...
    trainLog :+= "pos=%.0f neg=%.0f total=%.0f rate=%d/1000".format(pnum, totalNum - pnum, totalNum, rate)
    ...
    lr.save(spark.sparkContext, "/user/cpc/xgboost_lr/" + filetime)
    lr.clearThreshold()
    ...
    println(xgbtrain.first())
    val svmtrain = xgbtrain
      .map {
        x =>
    ...
    */
    ...
    if (upload > 0) {
      ...
      println(MUtils.updateMlcppOnlineData(filename, s"/home/work/mlcpp/data/$prefix.mlm", conf))
    }
  }
