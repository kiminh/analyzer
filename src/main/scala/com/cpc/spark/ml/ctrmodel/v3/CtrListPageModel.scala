...
/**
  ...
  * All media and list page model
  */
object CtrListPageModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ctx = model.initSpark("cpc ctr list page model %s [%s]".format(mode, modelPath))
    ...
    val lrfilepath = "/data/cpc/anal/model/logistic_list_page_%s.txt".format(date)
    ...
    var irError = 0d
    val irfilepath = "/data/cpc/anal/model/isotonic_list_page_%s.txt".format(date)
    if (mode.endsWith("+ir")) {
    ...
    CUtils.sendMail(txt, "CTR list page model train " + result, Seq("cpc-rd@innotechx.com","rd@aiclk.com"))
