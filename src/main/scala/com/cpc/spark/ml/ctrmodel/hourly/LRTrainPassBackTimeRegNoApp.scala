...
import com.cpc.spark.ml.train.LRIRModelReg
...
/**
  ...
  */
object LRTrainPassBackTimeRegNoApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val trainSql =
      s"""
         ...
         |where (label=0 or label=1) and day<='2019-08-07'
       """.stripMargin
...
//    val trainDF = getLeftJoinData(train1DF, userAppIdx).cache()
    ...
    val testSql =
      s"""
         ...
         | , label
         |from dl_cpc.pass_back_label_test_qizhi
         |where (label=0 or label=1) and day>='2019-08-16'
       """.stripMargin
...
//    val testDF = getLeftJoinData(test1DF, userAppIdx).cache()
  ...
  def train(spark: SparkSession, parser: String, name: String, train: DataFrame, test: DataFrame, destfile: String, n: Double): Unit = {
    trainLog :+= "\n------train log--------"
    ...
    println("train total positive negative", tnum, pnum, nnum, rate)
    trainLog :+= "train size total=%.0f positive=%.0f negative=%.0f scaleRate=%d/1000".format(tnum, pnum, nnum, rate)

    val testnum = test.count().toDouble
    val ptestnum = test.filter(_.getAs[Int]("label") > 0).count().toDouble
    val ntestnum = testnum - ptestnum
    println("test total positive negative \n", testnum, ptestnum, ntestnum)
    trainLog :+= "test size total=%.0f positive=%.0f negative=%.0f \n".format(testnum, ptestnum, ntestnum)
