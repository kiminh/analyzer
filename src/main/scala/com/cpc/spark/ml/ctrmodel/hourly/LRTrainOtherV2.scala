...
object LRTrainOtherV2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val qttAll = queryRawDataFromUnionEventsDF.join(userAppIdx, Seq("uid"), "left_outer").cache()

//    qttAll.write.mode("overwrite").saveAsTable(s"dl_cpc.bs_wy_sample_$date")
    println("suceess save sample")

    model.clearResult()

    var parser = "ctrparser4"
    var parserName = s"${typeWord}-bs-ctrparser4-daily"
    var parserDestFile = s"${typeWord}-bs-ctrparser4-daily.lrm"
    if ("ctrparser8".equals(parserArg)){
      parser = "ctrparser8"
      parserName = s"${typeWord}-bs-ctrparser8-daily"
      parserDestFile = s"${typeWord}-bs-ctrparser8-daily.lrm"
    }
...
//    s"rm /home/cpc/anal/model/togo/*" !
  ...
  def train(spark: SparkSession, parser: String, name: String, ulog: DataFrame, destfile: String, n: Double, typeWordCtrOrCVr: String): Unit = {
    println("------train log--------")
    println("name = %s,parser = %s,destfile = %s".format(name,parser,destfile))
