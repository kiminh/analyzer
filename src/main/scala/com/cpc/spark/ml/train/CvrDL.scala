...
object CvrDL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val datapath = "%s/{%s}".format(inpath, pathSep.mkString(","))
    println(datapath)

    val svm = MLUtils.loadLibSVMFile(ctx.sparkContext, datapath)
      .randomSplit(Array(sampleRate, 1 - sampleRate), seed = new Date().getTime)
    val testSample = svm(1)

    val in = svm(0).toLocalIterator.map{
      x =>
        val d = new DataSet()
        val f = new NDArray()
        x.features.toArray.foreach {
          v =>
            f.add(v)
        }
        d.addFeatureVector(f, x.label.toInt)
        d
    }
