  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = spark.sql(behavior_sql)
      ...
      .select(
        ...
        hashSeq("m16", "string")($"interest_ad_words_1").alias("m16"),
        hashSeq("m17", "string")($"interest_ad_words_3").alias("m17")
      )
      .persist()
    ...
    data.coalesce(20).foreachPartition { p =>
      ...
      p.foreach { rec =>
        ...
        val uid = "d6_" + rec.getString(0)
        for (i <- 1 to data.columns.length) {
