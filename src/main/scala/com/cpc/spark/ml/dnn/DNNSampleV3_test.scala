...
object DNNSampleV3_test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val train = getSample(spark, date)
    ...
    train.repartition(1000)
      ...
      .save("/user/cpc/zhj/test/dnntrain-" + date)

    val test = train.sample(withReplacement = false, 0.02).persist()
    val tn = test.count

    println("总数据：total = %d, 正比例 = %.4f".format(tn * 50, test.where("label=array(1,0)").count.toDouble / tn))
  ...
  def getSample(spark: SparkSession, date: String): DataFrame = {
    ...
    data.join(uid, Seq("uid"), "left")
      ...
      .select($"label",
        ...
        expr("if(count is null, 0L, 1L)").alias("f26"),
