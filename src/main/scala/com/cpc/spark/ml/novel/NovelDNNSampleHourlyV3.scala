...
object NovelDNNSampleHourlyV3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    train.repartition(100)
      ...
      .save(s"/user/cpc/wy/dnn_novel_v3/dnntrain-$date-$hour")
    ...
    train.sample(withReplacement = false, 0.1).repartition(100)
      ...
      .format("tfrecords")
      .option("recordType", "Example")
      .save(s"/user/cpc/wy/dnn_novel_v3/dnntest-$date-$hour")
  ...
  def getSample(spark: SparkSession, date: String, hour: String): DataFrame = {
    ...
    val behavior_data = spark.read.parquet("/user/cpc/wy/novel_behavior_v3")
