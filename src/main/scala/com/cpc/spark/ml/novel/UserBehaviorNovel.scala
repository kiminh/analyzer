...
object UserBehaviorNovel {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(show_sql)
      ...
      .parquet(s"/warehouse/dl_cpc.db/cpc_user_behaviors_novel/load_date=$date")

    spark.sql(
      s"""
         |alter table dl_cpc.cpc_user_behaviors_novel add partition(load_date='$date')
         |location '/warehouse/dl_cpc.db/cpc_user_behaviors_novel/load_date=$date'
      """.stripMargin)
