...
object bs_log_report_v2 {
  def main(args: Array[String]): Unit = {
    ...
    val pbData = spark.sql(stmt)
      .select(f($"raw"))
      .toDF("searchid", "involved_group_num", "group_media_num", "group_region_num", "group_l_v_num", "group_os_type_num",
      ...
      .filter(expr("bslog.exptags like '%%bsfilterdetail%%'"))
  ...
  def f=udf{
    raw:String=>
      val up = NoticeLogBody.parseFrom(Encoding.base64Decoder(raw).toArray).toBuilder
