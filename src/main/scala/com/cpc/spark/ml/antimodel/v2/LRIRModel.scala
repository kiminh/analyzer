...
class LRIRModel {
  ...
  def test2(sample: RDD[(String, LabeledPoint)]): RDD[(String, Double,Double)] = {
    if (lrmodel == null) {
      throw new Exception("must train lr first")
    }
    val lr = lrmodel
    lr.clearThreshold()
    sample.map{case (uid, LabeledPoint(label, features)) => (uid, lr.predict(features), label)}
  }
    ...
    lrTestResults
      .map {
        x =>
          ...
          ((v, x._2.toInt), 1)//predict, lable,count
      }
      .reduceByKey((x, y) => x + y)
      .map {
        x =>
