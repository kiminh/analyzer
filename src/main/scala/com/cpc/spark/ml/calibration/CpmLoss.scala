...
object CpmLoss {
  def main(args: Array[String]): Unit = {
    ...
    val sql =
      s"""
         | select ctr_model_name,avg(sample_loss*(cpm+0.01)) as cpm_weight_loss
         | from
         | (
         | select cpm,cast(isclick as int) as isclick,cast(raw_ctr as int) raw_ctr,
         |if(isclick = 1, -log(raw_ctr/1000000),-log(1-raw_ctr/1000000)) as sample_loss, ctr_model_name
