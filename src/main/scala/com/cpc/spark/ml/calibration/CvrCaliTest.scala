...
object CvrCaliTest{
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sql2 = s"""
                 |select iscvr as isclick, raw_cvr as raw_ctr, cast(exp_cvr as bigint) as ectr, cvr_model_name, adslotid, cast(ideaid as string) ideaid,
                 |case when user_req_ad_num = 1 then '1'
                 |  when user_req_ad_num = 2 then '2'
                 |  when user_req_ad_num in (3,4) then '4'
                 |  when user_req_ad_num in (5,6,7) then '7'
                 |  else '8' end as user_req_ad_num
                 |  from dl_cpc.qtt_cvr_calibration_sample where dt = '2019-05-26' and hour ='12'
       """.stripMargin
