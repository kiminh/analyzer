...
object video_sample_v2 {
  ...
  def getSample(spark: SparkSession, date: String): DataFrame = {
    ...
    val video_sample_new = video_sample.join(ideaid_feature, Seq("ideaidhash"), "left_outer").select($"sample_idx",$"idx0",$"idx1",$"idx2",$"id_arr", $"label", $"dense", $"resource")
    val original_sample_new = original_sample.join(ideaid_feature, Seq("ideaidhash"), "left_outer").select($"sample_idx",$"idx0",$"idx1",$"idx2",$"id_arr", $"label", $"dense", $"resource")
    val result = original_sample_new.union(video_sample_new).
      select($"sample_idx", $"idx0", $"idx1", $"idx2", $"id_arr", $"label", $"dense", array(hash("f" + 73)($"resource")).alias("resourceidhash")).rdd.map{
      r =>
