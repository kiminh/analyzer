...
object dsp_samples_extract {
  ...
  def main(args: Array[String]): Unit = {
    ...
    original_sample = spark.read.format("tfrecords").option("recordType", "Example")
      .load(s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/realtime/adlist-v4-dsp/$today/$hour/$Type/part*")
      .select($"sample_idx", $"idx0", $"idx1", $"idx2", $"id_arr", $"label", $"dense", expr("dense[25]").alias("uidhash"))
    println("---------111111111----------")
    original_sample.show(10)
  }
