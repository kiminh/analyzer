...
object recall_prepare_training_samples {
  ...
  def getSample(spark: SparkSession, model_version: String, featureName: String, Type: String, date: String, date1: String): DataFrame = {
    ...
    original_sample = spark.read.format("tfrecords").option("recordType", "Example").load(s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/daily/$model_version/$date1/part*").
      select($"sample_idx",$"idx0",$"idx1",$"idx2",$"id_arr", $"label", $"dense",expr("dense[25]").alias("uidhash"),expr("dense[8]").alias("adtypehash")).
    where(s"adtypehash in ($adtype8_hash, $adtype10_hash)").select($"sample_idx",$"idx0",$"idx1",$"idx2",$"id_arr", $"label", $"dense", $"uidhash")
//    if(Type == "train"){
