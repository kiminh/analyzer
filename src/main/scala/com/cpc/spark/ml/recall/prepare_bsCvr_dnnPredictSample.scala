...
object prepare_bsCvr_dnnPredictSample {
  ...
  def getSample(spark: SparkSession, date: String): DataFrame = {
    ...
    */
    ...
    val table2=s"(select unit_id as unitid from (SELECT unit_id,SUM(cost) as cnt FROM adv.cost where cost>0 and date='$day' group by unit_id) t order by cnt desc limit 1000) as tmp2"
    ...
    spark.sql(sql)
      ...
      $"uid"
    ).repartition(60000).persist(StorageLevel.DISK_ONLY)
