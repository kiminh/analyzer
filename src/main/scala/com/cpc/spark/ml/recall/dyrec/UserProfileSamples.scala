...
object UserProfileSamples {
  ...
  def main(args: Array[String]): Unit = {
    ...
    getSample(spark, model_version, Type, today, oneday, hour).repartition(1000)
      ...
      .save(s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/realtime/adlist-v4user-profile-features/$today/$hour/$Type")
    val CountPathTmpName = s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/daily/adlist-v4user-profile-features/tmp/"
    val CountPathName = s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/realtime/adlist-v4user-profile-features/$today/$hour/$Type/count"
    val count = spark.read.format("tfrecords").option("recordType", "Example").load(s"hdfs://emr-cluster/user/cpc/aiclk_dataflow/realtime/adlist-v4user-profile-features/$today/$hour/$Type/part*").count()
    ...
    val result = original_sample.join(sample_new, Seq("uidhash"), "left_outer").
      select($"sample_idx", $"idx0", $"idx1", $"idx2", $"id_arr", $"label", $"dense",
        mkSparseFeature_m(multihot_feature_number)(array(
          hashSeq("ud102", "string")($"fea1").alias("fea1"),
          hashSeq("ud103", "string")($"fea2").alias("fea2"),
          hashSeq("ud104", "string")($"fea3").alias("fea3"),
          hashSeq("ud105", "string")($"fea4").alias("fea4"),
          hashSeq("ud106", "string")($"fea5").alias("fea5"),
          hashSeq("ud107", "string")($"fea6").alias("fea6"),
          hashSeq("ud108", "string")($"fea7").alias("fea7"),
          hashSeq("ud109", "string")($"fea8").alias("fea8"),
          hashSeq("ud110", "string")($"fea9").alias("fea9"),
