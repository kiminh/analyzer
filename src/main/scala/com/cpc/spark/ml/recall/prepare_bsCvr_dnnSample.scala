  ...
  def getSample(spark: SparkSession, date: String): DataFrame = {
    ...
    spark.sql(sql)
      ...
      .select($"label",
        ...
        hash("f5")($"adslot_type").alias("f1"),
        //hash("f6")($"adslotid").alias("f6"),
        hash("f7")($"sex").alias("f2"),
        //hash("f8")($"dtu_id").alias("f8"),
        hash("f9")($"adtype").alias("f3"),
        //hash("f10")($"interaction").alias("f10"),
        //hash("f11")($"bid").alias("f11"),
        hash("f12")($"ideaid").alias("f4"),
        hash("f13")($"unitid").alias("f5"),
        hash("f14")($"planid").alias("f6"),
        hash("f15")($"userid").alias("f7"),
        //hash("f16")($"is_new_ad").alias("f16"),
        hash("f17")($"adclass").alias("f8"),
        hash("f18")($"site_id").alias("f9"),
        hash("f19")($"os").alias("f10"),
        //hash("f20")($"network").alias("f20"),
        hash("f21")($"phone_price").alias("f11"),
        hash("f22")($"brand").alias("f12"),
        hash("f23")($"province").alias("f13"),
        hash("f24")($"city").alias("f14"),
        hash("f25")($"city_level").alias("f15"),
        hash("f26")($"uid").alias("f16"),
        hash("f27")($"age").alias("f17"),
