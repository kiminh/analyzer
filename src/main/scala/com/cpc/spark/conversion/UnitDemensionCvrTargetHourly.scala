...
/**
  ...
  * desc:广告单元维度转化目标表
  */
object UnitDemensionCvrTargetHourly {
  def main(args: Array[String]): Unit = {
    val date = args(0)
    val hour = args(1)
    val dbName = args(2)
    println(s"day=$date, hour=$hour, dbName=$dbName")
...
//    var advUrl = config.getString("mariadb.adv.url")
//    advProps.put("user", config.getString("mariadb.adv.user"))
//    advProps.put("password", config.getString("mariadb.adv.password"))
//    advProps.put("driver", config.getString("mariadb.adv.driver"))
...
//        .repartition(10).write.mode("overwrite").saveAsTable("test.ocpc_check_data20190628")
    ...
    val sqlRequest2 =
      s"""
         ...
         |FROM
         |  dl_cpc.unit_conversion_target_hourly
         |WHERE
         |  `date` = '$date'
         |AND
         |  `hour` = '$hour'
    ...
    val tableName = s"$dbName.dw_unitid_conversion_target_hourly"
    resultDF.show(10)
    resultDF
        .select(
          "userid",
