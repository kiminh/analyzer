  ...
  def main(args: Array[String]): Unit = {
    ...
    val Sql3 =
      s"""
         ...
         |p.is_control_cost,
         |p.hidden_budget,
         |p.hidden_budget_ratio,
         |(if(p.one_payment>0,p.one_payment,0)*p.ocpc_cvr_cnt) as payment,
         |p.day
         |from
         |(select
         |a.userid,
         ...
         |((a.ocpc_cost/a.ocpc_cvr_cnt)- a.cpagiven*1.2) as one_payment,
         |a.day
         |from
         |(select
         |day,
         ...
         |group by userid,unitid,conversion_goal )d on a.userid=d.userid and a.unitid=d.unitid and a.conversion_goal=d.conversion_goal) p
