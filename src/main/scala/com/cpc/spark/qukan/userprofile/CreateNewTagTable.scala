...
object CreateNewTagTable {
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 to dateRange) {
      ...
      val unionTable2: DataFrame = spark.table("dl_cpc.cpc_bd_adx_blob").filter(s"day = date_add('%s', %s) and uid <> ''".format(dateValue, i)).select("uid",  "timestamp", "interests")
      val UnionTable: DataFrame = unionTable2.join(unionTable1, Seq("uid", "timestamp"), "inner").select("uid", "sex", "interests")
        .groupBy("uid").agg("sex" -> "max", "interests" -> "max").withColumnRenamed("max(interests)", "interests").withColumnRenamed("max(sex)", "sex")
        .withColumn("isstudent", Udfs.checkStudent()(col("interests"))).withColumn("isstudent2", Udfs.checkStudent2()(col("interests")))
