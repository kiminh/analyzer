  ...
  def main(args: Array[String]): Unit = {
    ...
    val statSql =
      s"""
        ...
        |  left outer join
        |  (select searchid, label2 as iscvr from dl_cpc.ml_cvr_feature_v1 where $dtCondition) b on a.searchid = b.searchid
        |  left outer join
        |  (select searhcid, iscvr as label3 from dl_cpc.cpc_api_union_log where $dtCondition) c on a.searchid = c.searchid
        |group by ideaid, ocpc_log_dict['kvalue'], ocpc_log_dict['cpagiven'] cpagiven,
      """.stripMargin
