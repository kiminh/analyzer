...
with_uid_des_dir="hdfs://emr-cluster/user/cpc/fenghuabin/adlist-v4-latest"
des_dir="hdfs://emr-cluster/user/cpc/fenghuabin/adlist-v4-latest-no-uid"
...
spark-submit --master yarn --queue ${queue} \
    ...
    --jars $( IFS=$','; echo "${jars[*]}" ) \
    --class com.cpc.spark.ml.dnn.baseData.MakeTrainExamplesNoUid \
    ${randjar} ${src_dir} ${date_begin} ${date_end} ${des_dir} ${instances_file} ${test_data_src} ${test_data_des} ${test_data_week} ${with_uid_des_dir}
