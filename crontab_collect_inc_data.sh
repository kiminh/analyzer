...
map_base=${dir}/${curr_date}_base_instances
map_incr=${dir}/${curr_date}_incr_instances

if [[ ! -f ${map_base} ]]; then
    hadoop fs -get ${curr_base_model_instances} ${map_base} &
fi
if [[ ! -f ${map_incr} ]]; then
...
flag="false"
if [[ -f ${map_base} ]]; then
    printf "detected today's base model map instances file, continue...\n"
    flag="true"
fi
if [[ -f ${map_incr} ]]; then
    printf "detected today's incr model map instances file, continue...\n"
    flag="true"
fi

if [ "${flag}" != "true" ];then
    printf "no today's base or incr model instances detected, exiting...\n"
    rm ${shell_in_run}
    exit 0
fi
