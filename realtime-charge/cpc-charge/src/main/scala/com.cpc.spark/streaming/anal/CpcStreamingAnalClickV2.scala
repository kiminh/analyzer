...
object CpcStreamingAnalClickV2 {
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("v3.2 cpc streaming anal click topics = " + topics + " env :" + env)
    ...
    try {
      ...
      case t: Exception => System.err.println("connect redis exception" + t.getStackTrace)
        //20180529 warning added by zhj
        val ts = System.currentTimeMillis()
        val toKafka = new Data2Kafka()
        toKafka.setMessage(ts, mapInt = Seq(("connecting_redis_on_restart", 1)))
        toKafka.sendMessage(brokers, "cpc_charge_warning")
        toKafka.close()
        fromOffsets = Map()
    }
    ...
    if (!fromOffsets.isEmpty) {
      ...
      kafkaoffset.foreach(tp => {
        try {
          ...
          case t: Throwable =>
            t.printStackTrace()
            println("set kafkaoffset key failed" + t.printStackTrace()) // TODO: handle error
        }
      })
    }
    ...
    if (isPrintFailed.length() > 0) {
      parserData.filter(x => x._1 == false).count().map {
        case cnt =>
      ...
      parserData.filter(!_._1).foreachRDD(
        rdd => {
          ...
          if (parseFail > 0) {
            val ts = System.currentTimeMillis()
            val toKafka = new Data2Kafka()
            toKafka.setMessage(ts, mapInt = Seq(("parse_fail_num", parseFail.toInt)))
            toKafka.sendMessage(brokers, "cpc_charge_warning")
            toKafka.close()
          }
        }
