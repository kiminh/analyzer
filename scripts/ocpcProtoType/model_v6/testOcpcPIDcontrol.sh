...
$SPARK_HOME/bin/spark-submit --master yarn --queue $queue \
    ...
    --jars $( IFS=$','; echo "${jars[*]}" ) \
    --class com.cpc.spark.OcpcProtoType.model_v6.OcpcPIDcontrol \
    /home/cpc/wangjun/analyzer/target/scala-2.11/cpc-anal_2.11-0.1.jar ${date} ${hour} ${version} ${media} ${sampleHour} ${minCV} ${kp} ${ki} ${kd} ${exptag} ${hourInt}
